...
We need to import the usual parser modules, followed by {\em Monad} , {\em Data.Map} and some usual suspects w.r.t file handling and 
the {\em ByteString}
\begin{code}
...
from \XeT\ point of view, a valid literate program would consist of an optional prelude, followed by a  list of {\sl sections}.
...
With \XeT\ you develop your program in {\em  sections}. Each section consists of an exposition in \TEX\ or \CONTEXT , followed by code in the formal programming language,
...
From the above section, \XeT\ reads the \quotation{META} data and accordingly,
...
contains it as it is.  This is achieved in \XeT\ via , what is defined , as links. The way to define link is 
\starttyping
//L link(<identifier>);
\stoptyping
...
In \XeT\ we use the following directive to indicate the name of the program file.
\starttyping
...
parseIdentifier = do {
                       c <- validFirstChar;
                       xs <- many (letter <|> digit <|> (char '-') <|> (char '.'));
                       spaces;
                       return $ c:xs
                      }
...
progInfoToTeX (Link str) map = 
  B.concat[ B.pack  "\n\\vskip .5cm\\framed{",
            ...
            B.pack "]"  ,
            B.pack "}\n\n"]
             where refName =  B.pack $ elemName $ metaData $ mGet map str
...
printCodeFiles (cs@(CSection fName ps):rest) env =
  do B.writeFile fName  $ csToCode cs env
...
handleInput fileName =
 do (prel,csecs,env) <- parseInputFile (fileName ++ ".lxt")
    printCodeFiles csecs env
    printTeXFile (fileName ++ ".tex") 
                 prel
                 csecs 
                 env 
